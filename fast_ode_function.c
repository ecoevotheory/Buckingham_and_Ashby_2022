...
void dynamic(double *x, double *dxdt, struct PARAM *p){
    ...
    dxdt[0] = p->a*(1-(x[0]+x[1]+x[2]+x[3]))*(x[1]+p->f*x[3])-(p->bJ+p->g+p->betaJ*(p->hval*x[2]+x[3]))*x[0];
    dxdt[1] = p->g*x[0]-(p->bA+p->betaA*(p->hval*x[2]+x[3]))*x[1];
    dxdt[2] = p->betaJ*(p->hval*x[2]+x[3])*x[0]-(p->bJ+p->g+p->alpha)*x[2];
    dxdt[3] = p->g*x[2]+p->betaA*(p->hval*x[2]+x[3])*x[1]-(p->bA+p->alpha)*x[3];
}
