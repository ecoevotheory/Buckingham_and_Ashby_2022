/***********************************************************************************************************
 * [t,SJ,SA,IJ,IA,EQFLAG] = eco_dynamics_function_v4(t_max,a0,g0,c1a,c2a,c1g,c2g,beta0,alpha,betaJ,betaA,hval,f,eqtol,init_pop,strain_totalJ,strain_totalA)
 ***********************************************************************************************************/

/* Compile in Matlab using mex eco_dynamics_function_v4.c */
...
void dynamic(double *SJ, double *SA, double *IJ, double *IA,  double *DSJDT, double *DSADT, double *DIJDT, double *DIADT, double *betaJ, double *betaA, struct PARAM *p){
    ...
    for(i=0;i<p->strain_totalJ;i++){ 
        for (j=0; j<p->strain_totalA; j++){
	    N = N + IJ[i+j*p->strain_totalJ] + SJ[i+j*p->strain_totalJ]+IA[i+j*p->strain_totalJ] + SA[i+j*p->strain_totalJ];
	}
    }
    ...
    for(i=0;i<p->strain_totalJ;i++){ 
        for (j=0; j<p->strain_totalA; j++){
	    allinfecteds = allinfecteds + p->hval*IJ[i+j*p->strain_totalJ] + IA[i+j*p->strain_totalJ];
	}
    }
    ...
    for(i=0;i<p->strain_totalJ;i++){
	for (j=0; j<p->strain_totalA; j++){
	    DSJDT[i+j*p->strain_totalJ] = p->a0*(1-N)*(SA[i+j*p->strain_totalJ]+p->f*IA[i+j*p->strain_totalJ])-(1+p->g0*(1-p->c1g*(1-exp(-p->c2g*betaJ[i]))/(1-exp(-p->c2g)))+p->beta0*(1-betaJ[i])*allinfecteds)*SJ[i+j*p->strain_totalJ];
            DSADT[i+j*p->strain_totalJ] = p->g0*(1-p->c1g*(1-exp(-p->c2g*betaJ[i]))/(1-exp(-p->c2g)))*SJ[i+j*p->strain_totalJ]-((1+p->c1a*(1-exp(-p->c2a*betaA[j]))/(1-exp(-p->c2a)))+p->beta0*(1-betaA[j])*allinfecteds)*SA[i+j*p->strain_totalJ];
	    ...
	    DIADT[i+j*p->strain_totalJ] = p->g0*(1-p->c1g*(1-exp(-p->c2g*betaJ[i]))/(1-exp(-p->c2g)))*IJ[i+j*p->strain_totalJ]+p->beta0*(1-betaA[j])*allinfecteds*SA[i+j*p->strain_totalJ]-(p->alpha+(1+p->c1a*(1-exp(-p->c2a*betaA[j]))/(1-exp(-p->c2a))))*IA[i+j*p->strain_totalJ];
	}
    }
}
