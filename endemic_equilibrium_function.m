...
% 'fast_ode_function' is an ODE solver.
[t,y] = fast_ode_function(orig_tmax,aval,h,f,gval,bJval,bAval,betaJ,betaA,alpha,initvec);
...
if(any(range(y(t1:end,:))>tol))
    [t,y] = fast_ode_function(orig_tmax*9,aval,h,f,gval,bJval,bAval,betaJ,betaA,alpha,y(end,:));
    t=t+orig_tmax;
    
    % Check again and extend again...
    t2=find(t>t(end)*0.9,1);
    if(any(range(y(t2:end,:))>tol)) 
        [t,y] = fast_ode_function(orig_tmax*90,aval,h,f,gval,bJval,bAval,betaJ,betaA,alpha,y(end,:));
        t=t+orig_tmax*10;
        t3=find(t>t(end)*0.9,1);
        
        % Check again and extend again...
        if(any(range(y(t3:end,:))>tol))
            [t,y] = fast_ode_function(orig_tmax*900,aval,h,f,gval,bJval,bAval,betaJ,betaA,alpha,y(end,:));
            t=t+orig_tmax*100;
