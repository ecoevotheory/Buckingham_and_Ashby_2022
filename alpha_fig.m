...
for i=1:6
    ...
    % virulence.
    [vec,Jplotvec,Aplotvec,disprevvec1,totalpopvec1,disprevvec2,totalpopvec2]=alpha_fig_data(version,a0,g0,c1a,c2a,c1g,c2g,f,beta0);
    upperx=max(log2(vec));
    lowerx=min(log2(vec));
    ...
    yyaxis right
    plot(log2(vecpart1),totalpopvec1,':','linewidth',3,'color',[0.8,0.8,0.8])
    plot(log2(vecpart2),totalpopvec2,':','linewidth',3,'color',[0.8,0.8,0.8])
    plot(log2(vecpart1),diseasedensity1,'-','linewidth',3,'color',[0.95,0.95,0.95])
    plot(log2(vecpart2),diseasedensity2,'-','linewidth',3,'color',[0.95,0.95,0.95])
    ...
    % We want the resistance curves to be on the top:
    yyaxis right
    plot(log2(vecpart1),Jplotvecpart1,'r-','linewidth',3)
    plot(log2(vecpart2),Jplotvecpart2,'r-','linewidth',3)
    plot(log2(vecpart1),Aplotvecpart1,'b--','linewidth',3)
