function [vec,Jplotvec,Aplotvec,resJss,resAss,disprevvec,juvpropvec,totalpopvec]=f_fig_data(version)
...
% of values of the sterility virulence.
...
% We will vary f between 0 and 1:
vec=linspace(0,1,21);
numberof=length(vec);
...
% Find singular strategies for each value of the sterility virulence:
...
for i=1:numberof
    ...
    Jplotvec(i,1:length(sort(unique([resJstab(i,:),resJrep(i,:)]))))=sort(unique([resJstab(i,:),resJrep(i,:)]));
    Aplotvec(i,1:length(sort(unique([resAstab(i,:),resArep(i,:)]))))=sort(unique([resAstab(i,:),resArep(i,:)]));
    if ~isnan(Jplotvec(i,2)) && isnan(Jplotvec(i,3))
        Jplotvec(i,[2,3])=Jplotvec(i,[3,2]);
    end
    if ~isnan(Aplotvec(i,2)) && isnan(Aplotvec(i,3))
        Aplotvec(i,[2,3])=Aplotvec(i,[3,2]);
    end
